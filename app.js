...
function markDirty() {
  ...
  autoSaveTimer = setTimeout(saveDatabase, 15000); // Auto-Save nach 15s
}
...
function createNewLanguage() {
  ...
  document.getElementById('cb_language').value = index_listLang;
...
function newVocRep() {
  ...
  // Max ID holen
  const maxRes = db.exec(`SELECT MAX(id) FROM "${lang}"`);
  const id_max = maxRes[0]?.values[0][0] || 1;

  // Zufällige Vokabel mit score >= scoreMinFilter suchen (analog Python retry-loop)
  let found = null;
  let attempts = 0;
  const maxAttempts = 200;

  // Effizienter: direkt alle passenden IDs laden, dann zufällig wählen
  ...
  // Antwort-Felder leeren / verstecken
  hideAnswer();
  // Score anzeigen (versteckt bis Antwort gezeigt)
...
async function importFromXlsx(file) {
  ...
  // Blatt "Tabelle2" analog Python, sonst erstes Blatt
  const sheetName = wb.SheetNames.includes('Tabelle2') ? 'Tabelle2' : wb.SheetNames[0];
  const ws = wb.Sheets[sheetName];
  const json = XLSX.utils.sheet_to_json(ws, { header: 1, defval: '' });
  // Ab Zeile 3 (Index 2) wie in Python
  xlsxRows = json.slice(2).filter(r => r[0] || r[1]);
  xlsxIndex = 0;
  if (xlsxRows.length === 0) {
    showStatus('Keine Daten in Tabelle2 ab Zeile 3 gefunden', 'err');
    return;
  }
  showImportRow();
...
function showImportRow() {
  ...
  // Spalte A = Bedeutung, Spalte B = Vokabel (analog Python)
...
async function init() {
  ...
  // DB laden: zuerst localStorage, sonst leere DB
